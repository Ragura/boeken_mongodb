...
const unsetNull = require("../plugins/unsetNull");
// const omitNull = require("../plugins/omitNull");
...
// boekSchema.pre("findOneAndUpdate", function() {
//     // console.log("hoi", this);

//     this._update.genres = this._update.genres.map(genre => genre.toLowerCase());
// });

// boekSchema.plugin(omitNull);
boekSchema.plugin(unsetNull);
...
boekSchema.statics.joiValidate = function(boek) {
  const joiSchema = Joi.object().keys({
    ...
    genres: Joi.array().items(
      Joi.string().valid(["romantisch", "horror", "fantasy"])
    ),
